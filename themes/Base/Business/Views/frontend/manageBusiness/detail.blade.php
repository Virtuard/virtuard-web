...
@section('content')
    <h2 class="title-bar no-border-bottom">
        {{$row->id ? __('Edit: ').$row->title : __('listing.business.add_new')}}
    </h2>
    ...
    <div class="lang-content-box">
        <form action="{{route('business.vendor.store',['id'=>($row->id) ? $row->id : '-1','lang'=>request()->query('lang')])}}" method="post" enctype="multipart/form-data">
            @csrf
            <div class="form-add-service">
                ...
                <div class="tab-content" id="nav-tabContent">
                    <div class="tab-pane fade show active" id="nav-tour-content">
                        @include('Business::admin/business/content')
                        @if(is_default_lang())
                    ...
                    <div class="tab-pane fade" id="nav-tour-location">
                        @include('Business::admin/business/location',["is_smart_search"=>"1"])
                    ...
                    <div class="tab-pane fade" id="nav-tour-pricing">
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <div class="form-group">
                                            @if(str_contains(url()->current(), 'edit'))
                                                <a href="{{ route('business.vendor.availability.index', ['id' => $row->id]) }}" class="btn btn-warning btn-sm"><i class="fa fa-calendar"></i> {{  __('Availability Business') }}</a>
                                            @else
                        ...
                        @include('Business::admin/business/pricing')
                    </div>
                    @if(is_default_lang())
                        <div class="tab-pane fade" id="nav-attribute">
                            @include('Business::admin/business/attributes')
                        </div>
...
@push('js')
    ...
    <script>
        $(".business-title").on("input",function(){
