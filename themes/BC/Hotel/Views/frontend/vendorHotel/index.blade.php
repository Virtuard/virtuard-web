...
@section('content')
    <h2 class="title-bar">
        {{!empty($recovery) ?__('Recovery Hotels') : __("listing.accomodation.manage")}}
        @if(Auth::user()->hasPermission('hotel_create') && empty($recovery))
            <a href="{{ route("hotel.vendor.create") }}" class="btn-change-password">{{__("listing.accomodation.add")}}</a>
        @endif
    </h2>
