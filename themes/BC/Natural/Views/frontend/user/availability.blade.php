    ...
    <div class="language-navigation">
        <div class="panel-body">
            <div class="filter-div d-flex justify-content-between ">
                ...
                <div class="col-right">
                    @if($rows->total() > 0)
                        <span class="count-string">{{ __("Showing :from - :to of :total naturals",["from"=>$rows->firstItem(),"to"=>$rows->lastItem(),"total"=>$rows->total()]) }}</span>
                    @endif
                </div>
            </div>
        </div>
    ...
    <script>
        ...
        $('#items_tab').on('show.bs.tab',function (e) {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events:{
                    url:"{{route('natural.vendor.availability.loadDates')}}",
                    extraParams:{
                        id:lastId,
                    }
                },
                ...
                eventClick:function (info) {
                    var form = Object.assign({},info.natural.extendedProps);
                    form.start_date = moment(info.natural.start).format('YYYY-MM-DD');
                    form.end_date = moment(info.natural.start).format('YYYY-MM-DD');
                    console.log(form);
                    formModal.show(form);
                },
                eventRender: function (info) {
                    $(info.el).find('.fc-title').html(info.natural.title);
                    $(info.el).find('.fc-content').attr("data-html","true").attr("title",info.natural.title).tooltip({ boundary: 'window' })
                }
            });
            calendar.render();
        });
        ...
        formModal = new Vue({
            ...
            methods:{
                ...
                saveForm:function () {
                    ...
                    $.ajax({
                        url:'{{route('natural.vendor.availability.store')}}',
                        data:this.form,
                        dataType:'json',
                        method:'post',
