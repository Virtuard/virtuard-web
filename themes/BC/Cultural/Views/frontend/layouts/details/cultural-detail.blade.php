<div class="g-header">
    ...
    <div class="right">
        @if(setting_item('cultural_enable_review') and $review_score)
...
@if(!empty($row->duration) or !empty($row->category_cultural->name) or !empty($row->max_people) or !empty($row->location->name))
    ...
    <div class="row">
        ...
        @endif
        @if(!empty($row->category_cultural->name))
            @php $cat =  $row->category_cultural->translate() @endphp
            <div class="col-xs-6 col-lg-3 col-md-6">
                <div class="item">
                    <div class="icon">
                    ...
                    <div class="info">
                        <h4 class="name">{{__("Cultural Type")}}</h4>
                        <p class="value">
                            {{$cat->name ?? ''}}
                        </p>
                    </div>
...
@endif
@include('Cultural::frontend.layouts.details.cultural-include-exclude')
@include('Cultural::frontend.layouts.details.cultural-itinerary')
@include('Cultural::frontend.layouts.details.cultural-attributes')
@include('Cultural::frontend.layouts.details.cultural-faqs')
@includeIf("Hotel::frontend.layouts.details.hotel-surrounding")

@if($row->map_lat && $row->map_lng)
<div class="g-location">
    <div class="location-title">
        <h3>{{__("Cultural Location")}}</h3>
        @if($translation->address)
            <div class="address">
