<?php
//if(!setting_item('cultural_enable_inbox')) return;
$vendor = $row->author;
?>
@if(!empty($vendor->id))
        ...
        <div class="media-body">
            <h4 class="media-heading"><a class="author-link" href="{{route('user.profile',['id'=>$vendor->user_name ?? $vendor->id])}}">{{$vendor->getDisplayName()}}</a>
                @if($vendor->is_verified)
                    <img loading='lazy'data-toggle="tooltip" data-placement="top" src="{{asset('icon/ico-vefified-1.svg')}}" title="{{__("Verified")}}" alt="{{__("Verified")}}">
                @else
                    <img loading='lazy'data-toggle="tooltip" data-placement="top" src="{{asset('icon/ico-not-vefified-1.svg')}}" title="{{__("Not verified")}}" alt="{{__("Verified")}}">
                @endif
            </h4>
