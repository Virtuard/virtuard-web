...
class Cultural extends Bookable
{
    use Notifiable;
    use SoftDeletes;
    use CapturesService;

    protected $table                              = 'bravo_culturals';
    public    $checkout_booking_detail_file       = 'Cultural::frontend/booking/detail';
    public    $checkout_booking_detail_modal_file = 'Cultural::frontend/booking/detail-modal';
    public    $set_paid_modal_file                = 'Cultural::frontend/booking/set-paid-modal';
    public    $email_new_booking_file             = 'Cultural::emails.new_booking_detail';
    public    $type                               = 'cultural';
    public    $availabilityClass                  = CulturalDate::class;
    protected $translation_class = CulturalTranslation::class;
     ...
     */
    public function category_cultural()
    {
        return $this->hasOne("Modules\Cultural\Models\CulturalCategory", "id", 'category_id')->with(['translation']);
    }
    ...
    {
        $meta['seo_title'] = __("Search for Culturals");
        if (!empty($title = setting_item_with_lang("cultural_page_list_seo_title", false))) {
            $meta['seo_title'] = $title;
        } else if (!empty($title = setting_item_with_lang("cultural_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("cultural_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        } else if (!empty($title = setting_item("cultural_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("cultural_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("cultural_page_list_seo_share");
    ...
    {
        return $this->hasOne($this->culturalMetaClass, "cultural_id");
    }
     ...
     */
    public function cultural_term()
    {
        return $this->hasMany($this->culturalTermClass, "cultural_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('cultural.cultural_route_prefix') . "/" . $this->slug;
        if (!empty($param)) {
    ...
    {
        return route('cultural.admin.edit',['id'=>$this->id]);
    }
    ...
    {
        $meta = $this->culturalMetaClass::where('cultural_id', $this->id)->first();
        if (!$meta) {
            $meta = new $this->culturalMetaClass();
            $meta->cultural_id = $this->id;
        }
        $arg = $request->input();
    ...
    public function addToCart(Request $request)
    {
        ...
        $total_buyer_fee = 0;
        if (!empty($list_buyer_fees = setting_item('cultural_booking_buyer_fees'))) {
    ...
    {
        $datesRaw = $this->culturalDateClass::getDatesInRanges($start_date, $this->id);
        $dates = [
            'base_price'   => null,
            'person_types' => null,
        ];
    ...
    {
        $culturalDate = $this->culturalDateClass::where('target_id', $this->id)->where('start_date', $start_date)->where('active', 1)->first();
        ...
        $maxGuests = !empty($culturalDate->max_guests) ? $culturalDate->max_guests : $this->max_people;
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if($this->isFixedDate()){
            if(Carbon::parse($request->start_date.' 00:00:00') <= $this->last_booking_date){
                return $this->sendError(__("This cultural is not available at selected dates"));
            }
        }else{
            ...
            if ($meta) {
                ...
                if ($meta->enable_open_hours) {
                    ...
                    if (!isset($open_hours[$nDate]) or empty($open_hours[$nDate]['enable'])) {
                        return $this->sendError(__("This cultural is not open on your selected day"));
                    }
                }
            }
    ...
    public function isAvailableInRanges($start_date){
        ...
        {
            $notAvailableDates = $this->culturalDateClass::query()->where([
            ...
            $availableDates = $this->culturalDateClass::query()->where([
                ['start_date','>=',$start_date],
                ['end_date','<=',$start_date],
                ['active','=',1],
                ['target_id','=',$this->id],
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('cultural_booking_buyer_fees');
        if (!empty($list_fees)) {
    ...
    {
        return setting_item("cultural_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("cultural_review_approved", 0);
    }

    public function review_after_booking(){
        return setting_item("cultural_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("cultural_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("cultural_review_stats", []))) {
    ...
    public function getReviewDataAttribute()
    {
        ...
        $dataTotalReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $this->id)->where('object_model', "Cultural")->where("status", "approved")->first();
     ...
     * Get Score Review
     *
     * Using for loop cultural
     */
    public function getScoreReview()
    {
        $cultural_id = $this->id;
        $list_score = Cache::rememberForever('review_' . $this->type . '_' . $cultural_id, function () use ($cultural_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $cultural_id)->where('object_model', "Cultural")->where("status", "approved")->first();
            return [
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if (!empty($location)) {
            ...
            })->where("bravo_culturals.status", "publish")->with(['translation'])->count("bravo_culturals.id");
        }
        if (empty($number))
            return false;
        if ($number > 1) {
            return __(":number Culturals", ['number' => $number]);
        }
        return __(":number Cultural", ['number' => $number]);
    }

    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','author_id'])
            ->where('object_id', $this->id)
            ->where('object_model', 'cultural')
            ->where("status", "approved")
            ->orderBy("id", "desc")
            ->with('author')
            ->paginate(setting_item('cultural_review_number_per_page', 5));
    }
    ...
    public function getBookingsInRange($from, $to)
    {
        ...
        $query->where('object_model', 'cultural');
    ...
    public function saveCloneByID($clone_id)
    {
        ...
        if (empty($old))
            return false;
        $selected_terms = $old->cultural_term->pluck('term_id');
        ...
        //Language
        $langs = $this->culturalTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {
        ...
        //Meta
        $metaCultural = $this->culturalMetaClass::where('cultural_id', $old->id)->first();
        if (!empty($metaCultural)) {
            $metaCulturalNew = $metaCultural->replicate();
            $metaCulturalNew->cultural_id = $new->id;
            $metaCulturalNew->save();
        }
    }
    ...
    {
        return setting_item('cultural_disable') == false;
    }

    public function isDepositEnable()
    {
        return (setting_item('cultural_deposit_enable') and setting_item('cultural_deposit_amount'));
    }

    public function getDepositAmount()
    {
        return setting_item('cultural_deposit_amount');
    }

    public function getDepositType()
    {
        return setting_item('cultural_deposit_type');
    }

    public function getDepositFomular()
    {
        return setting_item('cultural_deposit_fomular', 'default');
    }

    public static function isEnableEnquiry()
    {
        if (!empty(setting_item('booking_enquiry_for_cultural'))) {
            return true;
        }
        return false;
    }
    ...
    {
        $check = setting_item('booking_enquiry_for_cultural');
        if (!empty($check) and setting_item('booking_enquiry_type_cultural') == "booking_and_enquiry") {
            return true;
        }
        return false;
    }
    ...
    public static function getBookingEnquiryType()
    {
        $check = setting_item('booking_enquiry_for_cultural');
        if (!empty($check)) {
            if (setting_item('booking_enquiry_type_cultural') == "only_enquiry") {
                return "enquiry";
            }
        }
        return "book";
    ...
    {
        $model_cultural = parent::query()->select("bravo_culturals.*");
        $model_cultural->where("bravo_culturals.status", "publish");
        if (!empty($location_id = $request['location_id'] ?? "" )) {
            $location = Location::where('id', $location_id)->where("status", "publish")->first();
            if (!empty($location)) {
                $model_cultural->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_culturals.location_id')->where('bravo_locations._lft', '>=', $location->_lft)->where('bravo_locations._rgt', '<=', $location->_rgt);
                });
            } else {
                $model_cultural->where('address', 'LIKE', '%' . $location_id . '%')->get();
            }
        }
        if (!empty($price_range = $request['price_range'] ?? "")) {
            $pri_from = explode(";", $price_range)[0];
            $pri_to = explode(";", $price_range)[1];
            $raw_sql_min_max = "( (IFNULL(bravo_culturals.sale_price,0) > 0 and bravo_culturals.sale_price >= ? ) OR (IFNULL(bravo_culturals.sale_price,0) <= 0 and bravo_culturals.price >= ?) )
								AND ( (IFNULL(bravo_culturals.sale_price,0) > 0 and bravo_culturals.sale_price <= ? ) OR (IFNULL(bravo_culturals.sale_price,0) <= 0 and bravo_culturals.price <= ?) )";
            $model_cultural->WhereRaw($raw_sql_min_max, [
                $pri_from,
                $pri_from,
                $pri_to,
                $pri_to
        ...
        $list_cat = CulturalCategory::whereIn('id', $category_ids)->where("status", "publish")->get();
        foreach ($list_cat as $index=>$cat){
            $model_cultural->join('bravo_cultural_category as tc'.$index, function ($join) use ($cat,$index) {
                $join->on('tc'.$index.'.id', '=', 'bravo_culturals.category_id')
        ...
        if (is_array($terms) and !empty($terms = array_filter(array_values($terms)))) {
            foreach ($terms as $index=>$termId){
                $model_cultural->join('bravo_cultural_term as tt'.$index, function($join) use ($termId,$index){
                    $join->on('tt'.$index.'.cultural_id', "bravo_culturals.id");
        ...
        if (!empty($review_scores) && count($review_scores)) {
            ...
            foreach ($review_scores as $number) {
                $where_review_score[] = " ( bravo_culturals.review_score >= ? AND bravo_culturals.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_cultural->WhereRaw($sql_where_review_score,$params);
        }
        if (!empty($service_name = $request['service_name'] ?? [])) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $model_cultural->leftJoin('bravo_cultural_translations', function ($join) {
                    $join->on('bravo_culturals.id', '=', 'bravo_cultural_translations.origin_id');
                });
                $model_cultural->where('bravo_cultural_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $model_cultural->where('bravo_culturals.title', 'LIKE', '%' . $service_name . '%');
            }
        }
        if(!empty($lat = $request['map_lat'] ?? '') and !empty($lgn = $request['map_lgn'] ?? '') and !empty($request['map_place'] ?? '')){
            //			3959 - Miles(dặm), 6371 - Kilometers
                $distance  = setting_item('cultural_location_radius_value',0);
                if(!empty($distance) and setting_item('cultural_location_search_style')=='autocompletePlace'){
                    $distanceType = setting_item('cultural_location_radius_type',3959);
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
        ...
        {
            $model_cultural->where('bravo_culturals.is_featured',1);
        }
        ...
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_culturals.sale_price, 0 ) > 0 THEN bravo_culturals.sale_price ELSE bravo_culturals.price END AS tmp_min_price";
                $model_cultural->selectRaw($raw_sql);
                $model_cultural->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_culturals.sale_price, 0 ) > 0 THEN bravo_culturals.sale_price ELSE bravo_culturals.price END AS tmp_min_price";
                $model_cultural->selectRaw($raw_sql);
                $model_cultural->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
            ...
            default:
                if(!empty($request['order']) and !empty($request['order_by'])){
                    $model_cultural->orderBy("bravo_culturals.".$request['order'], $request['order_by']);
                }else{
                    $model_cultural->orderBy("is_featured", "desc");
                    $model_cultural->orderBy("id", "desc");
                }
        }

        $model_cultural->groupBy("bravo_culturals.id");
        return $model_cultural->with([
            'location',
            'hasWishList',
            'translation'
        ]);
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        if ($forSingle) {
            ...
            $data['category'] = CulturalCategory::selectRaw("id,name")->find($this->category_id) ?? null;
            ...
            $data['booking_fee'] = setting_item_array('cultural_booking_buyer_fees');
            ...
            $data['terms'] = Terms::getTermsByIdForAPI($this->cultural_term->pluck('term_id'));
        } else {
    ...
    {
        return "\Modules\Cultural\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        $category = CulturalCategory::selectRaw("id,name,slug")->where('status', 'publish')->with(['translation'])->get()->toTree();
        return [
            [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "4",
                "data" => Attributes::getAllAttributesForApi("Cultural")
            ]
        ];
    }
    ...
    {
        $search_fields = setting_item_array('cultural_search_fields');
