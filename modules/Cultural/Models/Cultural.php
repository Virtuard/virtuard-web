...
class Cultural extends Bookable
{
    ...
    {
        return $this->hasMany($this->culturalTermClass, "target_id");
    }
    ...
    {
        return url(route('cultural.admin.edit', ['id' => $this->id]));
    }
    ...
    public function getDataAvailableBooking($start_date)
    {
        ...
        $culturalDate = $this->culturalDateClass::where('target_id', $this->id)->where('start_date', $start_date)->first();
        if (!empty($culturalDate) and $culturalDate->active == 0) {
            return [];
        }
        if (!empty($culturalDate->ticket_types)) {
            $ticket_types = $culturalDate->ticket_types;
        }
    ...
    {
        return $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'vendor_id', 'author_id'])->where('object_id', $this->id)->where('object_model', 'cultural')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('cultural_review_number_per_page', 5));
    }
    ...
    public function search($request)
    {
        ...
        if (!empty($category_id = $request['category_id'] ?? "")) {
            $model_cultural->where('category_id', $category_id)->get();
        }
        if (!empty($location_id = $request['location_id'] ?? "")) {
            ...
            if (!empty($location)) {
                $model_cultural->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_culturals.location_id')
        ...
        if (!empty($price_range = $request['price_range'] ?? "")) {
            ...
            $model_cultural->WhereRaw($raw_sql_min_max, [$pri_from, $pri_from, $pri_to, $pri_to]);
        }
        ...
        if (is_array($terms) and !empty($terms = array_filter(array_values($terms)))) {
            foreach ($terms as $index => $termId) {
                $model_cultural->join('bravo_cultural_term as tt' . $index, function ($join) use ($termId, $index) {
                    $join->on('tt' . $index . '.target_id', "bravo_culturals.id");
        ...
        if ($max_guests) {
            $model_cultural->where('max_guests', '>=', $max_guests);
        }
        return $model_cultural->with(['location', 'hasWishList', 'translation']);
    }
    ...
    {
        return setting_item('cultural_booking_type', 'ticket');
    }
