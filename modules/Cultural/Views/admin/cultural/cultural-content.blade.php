<div class="panel">
    <div class="panel-title"><strong>{{ __('Cultural') }}</strong></div>
    <div class="panel-body">
        <div class="form-group">
            <label>{{ __('Title') }}</label>
            <input type="text" value="{!! clean($translation->title) !!}" placeholder="{{ __('Cultural title') }}" name="title" class="form-control">
        </div>
        <div class="form-group">
        ...
        @if (is_default_lang())
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{ __('Cultural Min People') }}</label>
                        <input type="text" name="min_people" class="form-control" value="{{ $row->min_people }}" placeholder="{{ __('Cultural Min People') }}">
                    </div>
                </div>
                <div class="col-lg-6">
                    <div class="form-group">
                        <label class="control-label">{{ __('Cultural Max People') }}</label>
                        <input type="text" name="max_people" class="form-control" value="{{ $row->max_people }}" placeholder="{{ __('Cultural Max People') }}">
                    </div>
                </div>
            </div>

        @endif
        <?php do_action(\Modules\Cultural\Hook::FORM_AFTER_MAX_PEOPLE, $row); ?>
        <div class="form-group-item">
                ...
                @if (!empty($translation->faqs))
                    ...
                    @foreach ($translation->faqs as $key => $faq)
                        <div class="item" data-number="{{ $key }}">
                            <div class="row">
                                <div class="col-md-5">
                                    <input type="text" name="faqs[{{ $key }}][title]" class="form-control" value="{{ $faq['title'] }}" placeholder="{{ __('Eg: When and where does the cultural end?') }}">
                                </div>
                                <div class="col-md-6">
            ...
            <div class="g-more hide">
                <div class="item" data-number="__number__">
                    <div class="row">
                        <div class="col-md-5">
                            <input type="text" __name__="faqs[__number__][title]" class="form-control" placeholder="{{ __('Eg: When and where does the cultural end?') }}">
                        </div>
                        <div class="col-md-6">
        ...
        @include('Cultural::admin/cultural/include-exclude')
        @include('Cultural::admin/cultural/itinerary')
        @if (is_default_lang())
            <div class="form-group">
