            ...
            <div class="d-flex" style="overflow-x: auto;">
                ...
                @foreach ($storyUsers as $ks => $story)
                    <div class="user-story" data-userId="{{ $story[0]->user_id }}">
                        @php
                            $isFullUrl = strpos($story[0]->media, 'http') === 0;
                            $type = get_file_type($story[0]->media);
                            $thumb = $isFullUrl ? $story[0]->media : (strpos($story[0]->media, 'story/') === 0 ? asset('storage/'.$story[0]->media) : asset('uploads/'.$story[0]->media));
                            if ($type != 'image') {
                ...
                <div class="story-popup" id="story-popup">
                    <div class="story-container">
                        ...
                        <div class="story-content" id="story-content">
                            ...
                            @endphp
                                @php 
                                    $isFullUrl = strpos($story->media, 'http') === 0;
                                    $mediaUrl = $isFullUrl ? $story->media : (strpos($story->media, 'story/') === 0 ? asset('storage/'.$story->media) : asset('uploads/'.$story->media));
                                @endphp
                                @if($storyType == 'image')
                                    <img loading='lazy' src="{{ $mediaUrl }}" class="story-item" data-id="{{ $story->id }}" data-userId="{{ $story->user_id }}" data-isowner="{{ $isOwner }}" data-type="image" />
                                @elseif($storyType == 'video')
                                    <video src="{{ $mediaUrl }}" class="story-item" data-id="{{ $story->id }}" data-userId="{{ $story->user_id }}" data-iswoner="{{ $isOwner }}" data-type="video"></video>
                                @endif
                            @endforeach
                            @endforeach
                        </div>
