            ...
            <div class="header-left">
                <a href="/" class="bravo-logo">
                    ...
                    @if($logo_id)
                        ...
                        <img src="{{$logo}}" alt="{{setting_item("site_title")}}" fetchpriority="high" decoding="async">
                    @endif
                </a>
            ...
            <div class="header-right">
                ...
                @if(!empty($header_right_menu))
                    <ul class="topbar-items">
                        ...
                        @if(!Auth::check())
                            <li class="login-item">
                                <a href="#login" data-toggle="modal" data-target="#login" class="login"
                                    aria-label="login">{{__('Login')}}</a>
                            </li>
                                ...
                                <li class="p-0">
                                    <a href="{{route('user.chat')}}">
                                        ...
                                        <sup class="badge badge-danger"
                                            style="top: -12px;">{{ auth()->user()->unseen_message_count }}</sup>
                                    </a>
                                </li>
                            @endif
                            ...
                            <li class="login-item dropdown">
                                <a href="#" data-toggle="dropdown" class="is_login">
                                    @if($avatar_url = Auth::user()->getAvatarUrl())
                                        <img loading='lazy' class="avatar" src="{{$avatar_url}}"
                                ...
                                <ul class="dropdown-menu text-left">
                                    @if(Auth::user()->hasPermission('dashboard_access'))
                                        <li class="menu-hr"><a href="{{route('admin.index')}}"><i class="icon ion-ios-ribbon"></i>
                                    ...
                                    <li class="@if(Auth::user()->hasPermission('dashboard_vendor_access')) menu-hr @endif">
                                        <a
                                            href="{{route('user.profile', ['id' => Auth::user()->user_name ?: Auth::user()->id])}}"><i
                                                class="icofont-user-suited"></i> {{__("My profile")}}</a>
                                    </li>
                                    <li class="@if(Auth::user()->hasPermission('dashboard_vendor_access')) menu-hr @endif">
                                        <a href="{{route('user.profile.setting')}}"><i class="icon ion-md-construct"></i>
                ...
                <div class="mobile-header">
                    <div class="mobile-icons d-md-none">
                        ...
                        @else
                            <a href="{{ route('user.profile', ['id' => Auth::user()->user_name ?: Auth::user()->id]) }}"
                                class="mobile-icon">
                                <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24">
                                    <path
                        ...
                        @if(Auth::check() && setting_item('inbox_enable'))
                            <a href="{{ route('user.chat') }}" class="mobile-icon">
                                ...
                                <span class="badge badge-danger"
                                    style="top: -5px; right: -10px;">{{ auth()->user()->unseen_message_count ?? 0 }}</span>
                            </a>
                        @endif
        ...
        <div class="user-profile">
            ...
            <ul>
                ...
                @else
                    ...
                    <li>
                        <a href="{{route('user.profile', ['id' => Auth::user()->user_name ?: Auth::user()->id])}}">
                            <i class="icofont-user-suited"></i> {{__("My profile")}}
                        </a>
                    </li>
                    <li>
                        <a href="{{route('user.profile.setting')}}">
                            <i class="icon ion-md-construct"></i> {{__("Profile Setting")}}
                        </a>
                    </li>
                    <li>
                        <a href="#"
