...
class CategoryController extends AdminController
{
    ...
    public function index(Request $request)
    {
        ...
        $listCategory = $this->spaceCategoryClass::query();
        ...
        $data = [
            ...
            'row'         => new $this->spaceCategoryClass(),
    ...
    {
        $this->checkPermission('space_manage_others');
        $row = $this->spaceCategoryClass::find($id);
        if (empty($row)) {
        ...
        $data = [
            ...
            'row'         => $row,
            'parents'     => $this->spaceCategoryClass::get()->toTree(),
            'breadcrumbs' => [
                [
                    'name' => __('Space'),
                    'url'  => route('space.admin.index')
    ...
    public function bulkEdit(Request $request)
    {
        ...
        if ($action == "delete") {
            foreach ($ids as $id) {
                $query = $this->spaceCategoryClass::where("id", $id)->first();
                if(!empty($query)){
                    //Sync child category
                    $list_childs = $this->spaceCategoryClass::where("parent_id", $id)->get();
            ...
            foreach ($ids as $id) {
                $query = $this->spaceCategoryClass::where("id", $id);
    ...
    public function getForSelect2(Request $request)
    {
        ...
        if($pre_selected && $selected){
            $item = $this->spaceCategoryClass::find($selected);
            if(empty($item)){
        ...
        $q = $request->query('q');
        $query = $this->spaceCategoryClass::select('id', 'name as text')->where("status","publish");
        if ($q) {
