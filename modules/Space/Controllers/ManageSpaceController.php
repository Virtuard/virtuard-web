...
class ManageSpaceController extends FrontendController
{
    ...
    public function storeApi(Request $request, $id = null)
    {
        if ($id > 0) {
            ...
            $this->checkPermission('space_create');
            $row = new $this->spaceClass();
            $row->status = setting_item('space_vendor_create_service_must_approved_by_admin', 0) ? 'pending' : 'publish';
            $row->author_id = Auth::id();
        }
        ...
        if ($request->hasFile('image_id')) {
            ...
            $path = $file->storeAs('/space', $filename);
        ...
        if ($request->hasFile('banner_image_id')) {
            ...
            $bannerPath = $banner->storeAs('/space', $filename);
        ...
        if ($res) {
            ...
            if ($id > 0) {
                event(new UpdatedServiceEvent($row));
                return response()->json(['message' => 'Space updated', 'data' => $row], 200);
            } else {
                event(new CreatedServicesEvent($row));
                return response()->json(['message' => 'Space created', 'data' => $row], 201);
            }
        }
