...
class Art extends Bookable
{
    use Notifiable;
    use SoftDeletes;
    use CapturesService;
    protected $table = 'bravo_arts';
    public $type = 'art';
    public $checkout_booking_detail_file       = 'Art::frontend/booking/detail';
    public $checkout_booking_detail_modal_file = 'Art::frontend/booking/detail-modal';

    public    $set_paid_modal_file                = 'Art::frontend/booking/set-paid-modal';
    public $email_new_booking_file             = 'Art::emails.new_booking_detail';
    protected $translation_class = ArtTranslation::class;
    ...
    public function __construct(array $attributes = [])
    {
        ...
        $this->artDateClass = ArtDate::class;
    ...
    {
        $meta['seo_title'] = __("Search for Arts");
        if (!empty($title = setting_item_with_lang("art_page_list_seo_title", false))) {
            $meta['seo_title'] = $title;
        } else if (!empty($title = setting_item_with_lang("art_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("art_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        } else if (!empty($title = setting_item("art_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("art_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("art_page_list_seo_share");
    ...
    {
        return $this->hasMany($this->artTermClass, "target_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . env('ART_ROUTE_PREFIX', 'art') . "/" . $this->slug;
        if (!empty($param)) {
    ...
    public static function getLinkForPageSearch($locale = false, $param = [])
    {
        ...
        return url(app_get_locale(false, false, '/') . env('ART_ROUTE_PREFIX', 'art') . "?" . http_build_query($param));
    }

    public function getEditUrl()
    {
        return url(route('art.admin.edit', ['id' => $this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $total_buyer_fee = 0;
        if (!empty($list_buyer_fees = setting_item('art_booking_buyer_fees'))) {
    ...
    public function getDataAvailableBooking($start_date)
    {
        ...
        $artDate = $this->artDateClass::where('target_id', $this->id)->where('start_date', $start_date)->first();
        if (!empty($artDate) and $artDate->active == 0) {
            return [];
        }
        if (!empty($artDate->ticket_types)) {
            $ticket_types = $artDate->ticket_types;
        }
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('art_booking_buyer_fees');
        if (!empty($list_fees)) {
    ...
    {
        return setting_item("art_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("art_review_approved", 0);
    }

    public function review_after_booking()
    {
        return setting_item("art_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("art_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("art_review_stats", []))) {
    ...
    {
        $art_id = $this->id;
        $list_score = Cache::rememberForever('review_' . $this->type . '_' . $art_id, function () use ($art_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $art_id)->where('object_model', "art")->where("status", "approved")->first();
    ...
    {
        return $this->reviewClass::select(['id', 'title', 'content', 'rate_number', 'author_ip', 'status', 'created_at', 'vendor_id', 'author_id'])->where('object_id', $this->id)->where('object_model', 'art')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('art_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Arts", ['number' => $number]);
        }
        return __(":number Art", ['number' => $number]);
    }
    ...
    public function saveCloneByID($clone_id)
    {
        ...
        //Language
        $langs = $this->artTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {
    ...
    {
        return setting_item('art_disable') == false;
    }
    ...
    {
        return (setting_item('art_deposit_enable') and setting_item('art_deposit_amount'));
    }
    public function getDepositAmount()
    {
        return setting_item('art_deposit_amount');
    }
    public function getDepositType()
    {
        return setting_item('art_deposit_type');
    }
    public function getDepositFomular()
    {
        return setting_item('art_deposit_fomular', 'default');
    }
    ...
    {
        if (!empty(setting_item('booking_enquiry_for_art'))) {
            return true;
        }
        return false;
    }
    ...
    {
        $check = setting_item('booking_enquiry_for_art');
        if (!empty($check) and setting_item('booking_enquiry_type_art') == "booking_and_enquiry") {
            return true;
        }
        return false;
    }
    public static function getBookingEnquiryType()
    {
        $check = setting_item('booking_enquiry_for_art');
        if (!empty($check)) {
            if (setting_item('booking_enquiry_type_art') == "only_enquiry") {
                return "enquiry";
            }
        }
        return "book";
    ...
    public function search($request)
    {
        ...
        if (!empty($location_id = $request['location_id'] ?? "")) {
            ...
            if (!empty($location)) {
                $model_art->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_arts.location_id')
                        ->where('bravo_locations._lft', '>=', $location->_lft)
                        ->where('bravo_locations._rgt', '<=', $location->_rgt);
                });
            } else {
                $model_art->where('address', 'LIKE', '%' . $location_id . '%')->get();
            }
        }
        if (!empty($price_range = $request['price_range'] ?? "")) {
            $pri_from = explode(";", $price_range)[0];
            $pri_to = explode(";", $price_range)[1];
            $raw_sql_min_max = "( (IFNULL(bravo_arts.sale_price,0) > 0 and bravo_arts.sale_price >= ? ) OR (IFNULL(bravo_arts.sale_price,0) <= 0 and bravo_arts.price >= ? ) )
                            AND ( (IFNULL(bravo_arts.sale_price,0) > 0 and bravo_arts.sale_price <= ? ) OR (IFNULL(bravo_arts.sale_price,0) <= 0 and bravo_arts.price <= ? ) )";
        ...
        if (is_array($terms) and !empty($terms = array_filter(array_values($terms)))) {
            foreach ($terms as $index => $termId) {
                $model_art->join('bravo_art_term as tt' . $index, function ($join) use ($termId, $index) {
                    $join->on('tt' . $index . '.target_id', "bravo_arts.id");
        ...
        if (is_array($review_scores) && !empty($review_scores)) {
            ...
            foreach ($review_scores as $number) {
                $where_review_score[] = " ( bravo_arts.review_score >= ? AND bravo_arts.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number . '.9';
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_art->WhereRaw($sql_where_review_score, $params);
        }

        if (!empty($service_name = $request["service_name"] ?? "")) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                $model_art->leftJoin('bravo_art_translations', function ($join) {
                    $join->on('bravo_arts.id', '=', 'bravo_art_translations.origin_id');
                });
                $model_art->where('bravo_art_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
                $model_art->where('bravo_arts.title', 'LIKE', '%' . $service_name . '%');
            }
        }

        if (!empty($lat = $request['map_lat'] ?? "") and !empty($lgn = $request['map_lgn'] ?? "") and !empty($request['map_place'])) {
            //			3959 - Miles(dặm), 6371 - Kilometers
            $distance  = setting_item('art_location_radius_value', 0);
            if (!empty($distance) and setting_item('art_location_search_style') == 'autocompletePlace') {
                $distanceType = setting_item('art_location_radius_type', 3959);
        ...
        if (!empty($request['is_featured'])) {
            $model_art->where('bravo_arts.is_featured', 1);
        }
        if (!empty($request['custom_ids'])) {
            $model_art->whereIn("bravo_arts.id", $request['custom_ids']);
        }
        if (!empty($request['square'])) {
            $model_art->where("bravo_arts.square", $request['square']);
        }
        if (!empty($request['room'])) {
            $model_art->where("bravo_arts.room", $request['room']);
        }
        if (!empty($request['bed'])) {
            $model_art->where("bravo_arts.bed", $request['bed']);
        }
        ...
        switch ($orderby) {
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_arts.sale_price, 0 ) > 0 THEN bravo_arts.sale_price ELSE bravo_arts.price END AS tmp_min_price";
                $model_art->selectRaw($raw_sql);
                $model_art->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_arts.sale_price, 0 ) > 0 THEN bravo_arts.sale_price ELSE bravo_arts.price END AS tmp_min_price";
                $model_art->selectRaw($raw_sql);
                $model_art->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
            ...
            default:
                if (!empty($request['order']) and !empty($request['order_by'])) {
                    $model_art->orderBy("bravo_arts." . $request['order'], $request['order_by']);
                } else {
                    $model_art->orderBy("is_featured", "desc");
                    $model_art->orderBy("id", "desc");
                }
        }

        $model_art->groupBy("bravo_arts.id");

        $max_guests = (int)(($request['adults'] ?? 0) + ($request['children'] ?? 0));
        if ($max_guests) {
            $model_art->where('max_guests', '>=', $max_guests);
        }
        return $model_art->with(['location', 'hasWishList', 'translation']);
    }
    ...
    public function dataForApi($forSingle = false)
    {
        ...
        if ($forSingle) {
            ...
            $data['booking_fee'] = setting_item_array('art_booking_buyer_fees');
    ...
    {
        return "\Modules\Art\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("art")
            ]
        ];
    }
    ...
    {
        return setting_item('art_booking_type', 'ticket');
    }
    ...
    {
        $search_fields = setting_item_array('art_search_fields');
