...
class AvailabilityController extends FrontendController
{
    ...
    protected $indexView = 'Natural::frontend.user.availability';
    ...
    public function index(Request $request)
    {
        ...
        $page_title = __('Naturals Availability');
    ...
    public function loadDates(Request $request)
    {
        ...
        $natural = $this->naturalClass::find($request->query('id'));
        if (empty($natural)) {
            return $this->sendError(__('Natural not found'));
        }
        $lang = app()->getLocale();
        $is_single = $request->query('for_single');
        $query = $this->naturalDateClass::query();
        ...
        foreach ($period as $dt){
            ...
            $date = [
                'id'           => rand(0, 999),
                'active'       => 0,
                'price'        => (!empty($natural->sale_price) and $natural->sale_price > 0 and $natural->sale_price < $natural->price) ? $natural->sale_price : $natural->price,
                'is_default'   => true,
                'textColor'    => '#2791fe',
            ];
            if (!$is_single) {
            ...
            $date['max_guests'] = $natural->max_people;
            $date['title_origin'] = $date['price_html'];
            $date['title'] = $date['event'] = $date['price_html'] . "<br>". __('Max guests: ') . $natural->max_people;
            ...
            if ($request->input('for_single')) {
                if (empty(!$natural->max_people) and $natural->max_people < 1) {
                    $date['active'] = 0;
                }
            }
            if (!empty($natural->meta->enable_person_types) and $natural->meta->enable_person_types == 1) {
                $date['person_types'] = $natural->meta->person_types;
            ...
            // Open Hours
            if (!empty($natural->meta->enable_open_hours) and $natural->meta->enable_open_hours == 1) {
                $open_hours = $natural->meta->open_hours;
                $nDate = date('N', $i);
        ...
        if (!empty($rows)) {
            foreach ($rows as $row) {
                ...
                if (!empty($natural->meta->enable_person_types) and $natural->meta->enable_person_types == 1) {
                    $list_person_types = $natural->meta->person_types;
        ...
        $bookings = $this->bookingClass::getBookingInRanges($natural->id, $natural->type, $request->query('start'), $request->query('end'));
        if (!empty($bookings)) {
        ...
        if (!empty($natural->ical_import_url)) {
            $startDate = $request->query('start');
            $endDate = $request->query('end');
            $timezone = setting_item('site_timezone', config('app.timezone'));
            try {
                $icalevents = new Ical($natural->ical_import_url, [
    ...
    public function store(Request $request)
    {
        ...
        $natural = $this->naturalClass::find($request->input('target_id'));
        $target_id = $request->input('target_id');
        if (empty($natural)) {
            return $this->sendError(__('Natural not found'));
        }
        if (!$this->hasPermission('natural_manage_others')) {
            if ($natural->author_id != Auth::id()) {
...
//        for ($i = strtotime($request->input('start_date')); $i <= strtotime($request->input('end_date')); $i += DAY_IN_SECONDS) {
        ...
        foreach ($period as $dt){
            $date = $this->naturalDateClass::where('start_date', $dt->format('Y-m-d'))->where('target_id', $target_id)->first();
            if (empty($date)) {
                $date = new $this->naturalDateClass();
