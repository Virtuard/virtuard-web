    ...
    public function loadDates(Request $request){
        ...
        foreach ($period as $dt){
            ...
            if ($natural->ticket_types and $natural->getBookingType() == "ticket") {
                $date['ticket_types'] = $natural->ticket_types;
                $c_title = "";
        ...
        if(!empty($rows))
        {
            foreach ($rows as $row)
            {
                ...
                if ($row->ticket_types and $natural->getBookingType() == "ticket") {
                    ...
                    $ticketData['title'] = $ticketData['natural']  = $c_title;
                    $ticketData['ticket_types'] = $list_ticket_types;
                }

                if ($natural->getBookingType() == "time_slot") {
                    if (!$is_single) {
                        $ticketData['title'] = $ticketData['natural'] = format_money_main($row['price']);
                    } else {
                        $ticketData['title'] = $ticketData['natural'] = format_money($row['price']);
                    }
        ...
        $bookings = $this->bookingClass::getAllBookingInRanges($natural->id,$natural->type,$request->query('start'),$request->query('end'));
        if(!empty($bookings))
        {
            foreach ($bookings as $booking){
                ...
                foreach ($period as $dt){
                    ...
                    if(isset($allDates[$date])){
                        ...
                        if($natural->getBookingType() == "ticket")
                        {
                            $c_title = "";
