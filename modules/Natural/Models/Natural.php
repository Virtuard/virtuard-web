...
class Natural extends Bookable
{
    ...
    {
        $meta['seo_title'] = __("Search for Naturals");
        if (!empty($title = setting_item_with_lang("natural_page_list_seo_title", false))) {
            $meta['seo_title'] = $title;
        } else if (!empty($title = setting_item_with_lang("natural_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("natural_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        } else if (!empty($title = setting_item("natural_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("natural_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("natural_page_list_seo_share");
    ...
    {
        return setting_item("natural_enable_review", 0);
    }
    ...
    public function count_remain_review()
    {
        ...
        $options = setting_item("natural_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("natural_review_stats", []))) {
    ...
    public function saveCloneByID($clone_id)
    {
        ...
        //Language
        $langs = $this->naturalTranslationClass::where("origin_id", $old->id)->get();
        if (!empty($langs)) {
    ...
    {
        if (!empty(setting_item('booking_enquiry_for_natural'))) {
            return true;
        }
        return false;
    }
    ...
    public static function isFormEnquiryAndBook()
    {
        ...
        if (!empty($check) and setting_item('booking_enquiry_type_natural') == "booking_and_enquiry") {
            return true;
        }
        return false;
    }
    ...
    public static function getBookingEnquiryType()
    {
        $check = setting_item('booking_enquiry_for_natural');
        if (!empty($check)) {
            if (setting_item('booking_enquiry_type_natural') == "only_enquiry") {
                return "enquiry";
            }
        }
        return "book";
    ...
    public function search($request)
    {
        ...
        if (!empty($service_name = $request['service_name'] ?? [])) {
            if (setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale()) {
                ...
                $model_natural->where('bravo_natural_translations.title', 'LIKE', '%' . $service_name . '%');
            } else {
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
        ...
        switch ($orderby){
            case "price_low_high":
                ...
                $model_natural->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_naturals.sale_price, 0 ) > 0 THEN bravo_naturals.sale_price ELSE bravo_naturals.price END AS tmp_min_price";
                $model_natural->selectRaw($raw_sql);
                $model_natural->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
    ...
    {
        return "\Modules\Natural\Controllers\AvailabilityController";
    }
    ...
    {
        $search_fields = setting_item_array('natural_search_fields');
