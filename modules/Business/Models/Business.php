...
class Business extends Bookable
{
    ...
    use SoftDeletes;
    use CapturesService;
    protected $table = 'bravo_businesses';
    public $type = 'business';
    public $checkout_booking_detail_file       = 'Business::frontend/booking/detail';
    public $checkout_booking_detail_modal_file = 'Business::frontend/booking/detail-modal';
    public $set_paid_modal_file                = 'Business::frontend/booking/set-paid-modal';
    public $email_new_booking_file             = 'Business::emails.new_booking_detail';
    public $availabilityClass = BusinessDate::class;
    protected $translation_class = BusinessTranslation::class;
    ...
    {
        $meta['seo_title'] = __("Search for Business");
        if (!empty($title = setting_item_with_lang("business_page_list_seo_title",false))) {
            $meta['seo_title'] = $title;
        }else if(!empty($title = setting_item_with_lang("business_page_search_title"))) {
            $meta['seo_title'] = $title;
        }
        $meta['seo_image'] = null;
        if (!empty($title = setting_item("business_page_list_seo_image"))) {
            $meta['seo_image'] = $title;
        }else if(!empty($title = setting_item("business_page_search_banner"))) {
            $meta['seo_image'] = $title;
        }
        $meta['seo_desc'] = setting_item_with_lang("business_page_list_seo_desc");
        $meta['seo_share'] = setting_item_with_lang("business_page_list_seo_share");
    ...
    public function terms(){
        return $this->hasMany($this->businessTermClass, "target_id");
    }
    ...
    public function getDetailUrl($include_param = true)
    {
        ...
        $urlDetail = app_get_locale(false, false, '/') . config('business.business_route_prefix') . "/" . $this->slug;
        if(!empty($param)){
    ...
    {
        return url(route('business.admin.edit',['id'=>$this->id]));
    }
    ...
    public function addToCart(Request $request)
    {
        ...
        $total_buyer_fee = 0;
        if (!empty($list_buyer_fees = setting_item('business_booking_buyer_fees'))) {
    ...
    public function getPriceInRanges($start_date,$end_date){
        ...
        $datesRaw = $this->businessDateClass::getDatesInRanges($start_date,$end_date,$this->id);
        $dates = [];
        if(!empty($datesRaw))
        {
    ...
    public function addToCartValidate(Request $request)
    {
        ...
        if(!$this->isAvailableInRanges($start_date,$end_date)){
            return $this->sendError(__("This business is not available at selected dates"));
        }
    ...
    public function isAvailableInRanges($start_date,$end_date){
        ...
        {
            $notAvailableDates = $this->businessDateClass::query()->where([
            ...
            $availableDates = $this->businessDateClass::query()->where([
                ['start_date','>=',$start_date],
                ['end_date','<=',$end_date],
                ['active','=',1],
                ['target_id','=',$this->id],
    ...
    public function getBookingData()
    {
        ...
        $list_fees = setting_item_array('business_booking_buyer_fees');
        if(!empty($list_fees)){
    ...
    {
        return setting_item("business_enable_review", 0);
    }

    public function getReviewApproved()
    {
        return setting_item("business_review_approved", 0);
    }

    public function review_after_booking(){
        return setting_item("business_enable_review_after_booking", 0);
    }

    public function count_remain_review()
    {
        $status_making_completed_booking = [];
        $options = setting_item("business_allow_review_after_making_completed_booking", false);
        if (!empty($options)) {
    ...
    {
        $reviewStats = [];
        if (!empty($list = setting_item("business_review_stats", []))) {
     ...
     * Get Score Review
     *
     * Using for loop business
     */
    public function getScoreReview()
    {
        $business_id = $this->id;
        $list_score = Cache::rememberForever('review_'.$this->type.'_' . $business_id, function () use ($business_id) {
            $dataReview = $this->reviewClass::selectRaw(" AVG(rate_number) as score_total , COUNT(id) as total_review ")->where('object_id', $business_id)->where('object_model', "business")->where("status", "approved")->first();
    ...
    public function getReviewList(){
        return $this->reviewClass::select(['id','title','content','rate_number','author_ip','status','created_at','vendor_id','author_id'])->where('object_id', $this->id)->where('object_model', 'business')->where("status", "approved")->orderBy("id", "desc")->with('author')->paginate(setting_item('business_review_number_per_page', 5));
    }
    ...
    public function getNumberServiceInLocation($location)
    {
        ...
        if ($number > 1) {
            return __(":number Business", ['number' => $number]);
        }
        return __(":number Business", ['number' => $number]);
    }
    ...
    public function saveCloneByID($clone_id){
        ...
        //Language
        $langs = $this->businessTranslationClass::where("origin_id",$old->id)->get();
        if(!empty($langs)){
    ...
    public static function isEnable(){
        return setting_item('business_disable') == false;
    }

    public function isDepositEnable(){
        return (setting_item('business_deposit_enable') and setting_item('business_deposit_amount'));
    }
    public function getDepositAmount(){
        return setting_item('business_deposit_amount');
    }
    public function getDepositType(){
        return setting_item('business_deposit_type');
    }
    public function getDepositFomular(){
        return setting_item('business_deposit_fomular','default');
    }
    ...
    public static function isEnableEnquiry(){
        if(!empty(setting_item('booking_enquiry_for_business'))){
            return true;
        }
        return false;
    }
    ...
    public static function isFormEnquiryAndBook(){
        $check = setting_item('booking_enquiry_for_business');
        if(!empty($check) and setting_item('booking_enquiry_type_business') == "booking_and_enquiry" ){
            return true;
        }
        return false;
    }
    public static function getBookingEnquiryType(){
        $check = setting_item('booking_enquiry_for_business');
        if(!empty($check)){
            if( setting_item('booking_enquiry_type_business') == "only_enquiry" ) {
                return "enquiry";
            }
        }
        return "book";
    ...
    public function search($request)
    {
        ...
        if (!empty($location_id = $request['location_id'] ?? '')) {
            ...
            if(!empty($location)){
                $model_business->join('bravo_locations', function ($join) use ($location) {
                    $join->on('bravo_locations.id', '=', 'bravo_businesses.location_id')
                        ->where('bravo_locations._lft', '>=', $location->_lft)
                        ->where('bravo_locations._rgt', '<=', $location->_rgt);
                });
            } else {
                $model_business->where('address', 'LIKE', '%' . $location_id . '%')->get();
            }
        }
        if (!empty($price_range = $request['price_range'] ?? '')) {
            $pri_from = explode(";", $price_range)[0];
            $pri_to = explode(";", $price_range)[1];
            $raw_sql_min_max = "( (IFNULL(bravo_businesses.sale_price,0) > 0 and bravo_businesses.sale_price >= ? ) OR (IFNULL(bravo_businesses.sale_price,0) <= 0 and bravo_businesses.price >= ? ) )
                            AND ( (IFNULL(bravo_businesses.sale_price,0) > 0 and bravo_businesses.sale_price <= ? ) OR (IFNULL(bravo_businesses.sale_price,0) <= 0 and bravo_businesses.price <= ? ) )";
        ...
        if (is_array($terms) and !empty($terms = array_filter($terms))) {
            $model_business->join('bravo_business_term as tt', 'tt.target_id', "bravo_businesses.id")->whereIn('tt.term_id', $terms);
        }
        ...
        if (!empty($review_scores) && count($review_scores)) {
            ...
            foreach ($review_scores as $number){
                $where_review_score[] = " ( bravo_businesses.review_score >= ? AND bravo_businesses.review_score <= ? ) ";
                $params[] = $number;
                $params[] = $number.'.9';
            }
            $sql_where_review_score = " ( " . implode("OR", $where_review_score) . " )  ";
            $model_business->WhereRaw($sql_where_review_score,$params);
        }

        if(!empty( $service_name = $request['service_name'] ?? ""  )){
            if( setting_item('site_enable_multi_lang') && setting_item('site_locale') != app()->getLocale() ){
                $model_business->leftJoin('bravo_business_translations', function ($join) {
                    $join->on('bravo_businesses.id', '=', 'bravo_business_translations.origin_id');
                });
                $model_business->where('bravo_business_translations.title', 'LIKE', '%' . $service_name . '%');

            }else{
                $model_business->where('bravo_businesses.title', 'LIKE', '%' . $service_name . '%');
            }
        }
...
//            ORDER BY (POW((lon-$lon),2) + POW((lat-$lat),2))";
        ...
        switch ($orderby){
            case "price_low_high":
                $raw_sql = "CASE WHEN IFNULL( bravo_businesses.sale_price, 0 ) > 0 THEN bravo_businesses.sale_price ELSE bravo_businesses.price END AS tmp_min_price";
                $model_business->selectRaw($raw_sql);
                $model_business->orderBy("tmp_min_price", "asc");
                break;
            case "price_high_low":
                $raw_sql = "CASE WHEN IFNULL( bravo_businesses.sale_price, 0 ) > 0 THEN bravo_businesses.sale_price ELSE bravo_businesses.price END AS tmp_min_price";
                $model_business->selectRaw($raw_sql);
                $model_business->orderBy("tmp_min_price", "desc");
                break;
            case "rate_high_low":
        ...
        if($max_guests){
            $model_business->where('max_guests','>=',$max_guests);
        }


        return $model_business->with(['location','hasWishList','translation']);
    }
    ...
    public function dataForApi($forSingle = false){
        ...
        if($forSingle){
            ...
            $data['booking_fee'] = setting_item_array('business_booking_buyer_fees');
    ...
    {
        return "\Modules\Business\Controllers\AvailabilityController";
    }
    ...
    static public function getFiltersSearch()
    {
        ...
        return [
            ...
            [
                ...
                "field"    => "terms",
                "position" => "3",
                "data" => Attributes::getAllAttributesForApi("business")
            ]
        ];
    }
    public static function getBookingType(){
        return setting_item('business_booking_type','by_day');
    }

    static public function getFormSearch()
    {
        $search_fields = setting_item_array('business_search_fields');
