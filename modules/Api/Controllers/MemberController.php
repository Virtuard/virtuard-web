...
class MemberController extends Controller
{
    ...
    public function allMembers(Request $request)
    {
        try {
            ...
            $perPage = min($perPage, 10000); // Allow up to 10000 per page for leaderboard
            ...
            // Sort by stats if requested
            $sort = $request->get('sort', 'last_login');
            if ($sort === 'total_score') {
                $query->orderBy('total_score', 'DESC');
            } elseif ($sort === 'levels') {
                $query->orderBy('completed_levels_count', 'DESC');
            } else {
            ...
            // Transform collection to add is_followed field and game stats
            $currentUserId = Auth::id();
            $members->getCollection()->transform(function ($user) use ($currentUserId) {
                $user->avatar_url = $user->getAvatarUrl();
                $user->bio = $user->clean_bio; // Usa la bio pulita senza HTML
                ...
                // Game Stats (Aggiunte esplicitamente per garantire che vengano inviate nelle liste)
                $user->current_level = (int) $user->current_level;
                $user->coins = (int) $user->coins;
                $user->total_score = (int) $user->total_score;
                $user->trophies_count = is_array($user->trophies) ? count($user->trophies) : (int) $user->trophies;
                $user->completed_levels_count = (int) $user->completed_levels_count;
            ...
            return $this->sendError('Failed to retrieve members', []);
        }
    }
        ...
        try {
            ...
            $data = [
                'member' => [
                    ...
                    'email' => $user->email,
                    'bio' => $user->clean_bio, // Usa la bio pulita
                    ...
                    'custom_trophies' => $user->gameProgress->trophies ?? [],
                    'total_play_time' => (int) $user->total_play_time,
                    'completed_levels_count' => (int) $user->completed_levels_count,
                    'puzzle_details' => $user->gameProgress->completed_levels_data ?? [],
            ...
            return $this->sendError('Failed to retrieve member detail', []);
        }
    }
    ...
    public function follow(Request $request, $id)
    {
        try {
            ...
            if (!$targetUser || $targetUser->status != 'publish') {
                return $this->sendError('Member not found', []);
            }
            ...
            if ($existingFollow) {
                return $this->sendError('Already following this member', []);
            }
            ...
            return $this->sendError('Failed to follow member', []);
        }
    }
    ...
    public function unfollow(Request $request, $id)
    {
        try {
            ...
            if (!$existingFollow) {
                return $this->sendError('Not following this member', []);
            }
            ...
            return $this->sendError('Failed to unfollow member', []);
        }
    }
