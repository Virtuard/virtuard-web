...
class ProfileController extends Controller
{
    ...
    public function getProfile(Request $request)
    {
        try {
            ...
            $user = $request->user();
            $user->loadMissing(['gameProgress']);

            \Illuminate\Support\Facades\Log::info("Profile Sync Debug User #{$user->id}:", [
                'raw_bio' => $user->bio,
                'clean_bio' => $user->clean_bio,
                'vard_stats' => $user->vard_stats,
                'game_progress' => $user->gameProgress ? $user->gameProgress->toArray() : 'null'
            ]);
