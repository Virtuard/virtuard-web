...
class UserGameProgressController extends Controller
{
    ...
    public function uploadImage(Request $request)
    {
        try {
            ...
            if (!$file || !$file->isValid()) {
                return $this->sendError('Invalid image file provided', [], 422);
            }
    ...
    public function players(Request $request)
    {
        try {
            ...
            // Query all active users, optionally with game progress
            ...
            // To sort by progress fields (which are in a relation), we must join the table for sorting
            $query->leftJoin('user_game_progress', 'users.id', '=', 'user_game_progress.user_id')
                ->select('users.*', 'user_game_progress.current_level as pg_level', 'user_game_progress.coins as pg_coins');

            // Map allowed sort fields to tables
            $progressSortFields = ['total_score', 'experience', 'current_level', 'total_play_time'];
            if (in_array($sortBy, $progressSortFields)) {
                // If sort field is missing in progress table, check the users table columns (fallback)
                $query->orderByRaw("COALESCE(user_game_progress.$sortBy, users.$sortBy, 0) $sortOrder");
            } else {
            ...
            if ($sortBy !== 'created_at') {
                ...
                $query->orderByRaw("COALESCE(user_game_progress.total_score, 0) desc");
            }

            $usersList = $query->paginate($perPage);

            // Transform collection to include user and game progress data
            $players = collect($usersList->items())->map(function ($user) {
                ...
                return [
                    ...
                    'game_progress' => $progress ? [
                        ...
                        'current_checkpoint' => (int) $progress->current_checkpoint,
                        'total_score' => (int) $progress->total_score,
                        'experience' => (int) $progress->experience,
                        'coins' => (int) $progress->coins,
                        'lives' => (int) $progress->lives,
                        'total_play_time' => (int) $progress->total_play_time,
                        'completed_levels_count' => (function ($data) {
                            if (is_array($data))
                                return count($data);
                            if (is_string($data)) {
                            ...
                            return 0;
                        })($progress->completed_levels_data),
                        'trophies_count' => (function ($data) {
                            if (is_array($data))
                                return count($data);
                            if (is_string($data)) {
                                $decoded = json_decode($data, true);
                        ...
                        'lives' => 5,
                        'total_play_time' => (int) $user->total_play_time,
                        'completed_levels_count' => (int) $user->completed_levels_count,
                        'trophies_count' => is_array($user->trophies) ? count($user->trophies) : (int) $user->trophies,
                        'last_played_at' => $user->created_at,
                        'first_played_at' => $user->created_at,
                    ],
                    'vard_stats' => $user->vard_stats,
            ...
            return $this->sendSuccess([
                'players' => $players,
                'pagination' => [
                    'current_page' => $usersList->currentPage(),
                    'per_page' => $usersList->perPage(),
                    'total' => $usersList->total(),
                    'last_page' => $usersList->lastPage(),
                    'from' => $usersList->firstItem(),
                    'to' => $usersList->lastItem(),
                    'has_more_pages' => $usersList->hasMorePages()
                ]
