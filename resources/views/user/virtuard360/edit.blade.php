...
@push('js')
    ...
    <script>
        ...
        function checkPanoramaImagesAndProtectSave() {
    let panoramaId = "{{ $panorama->id ?? null }}";
    if (!panoramaId) return;

    fetch(`/user/virtuard-360/add/api/edit?id=${panoramaId}`)
        .then(res => res.json())
        .then(res => {
            let hasImage = res.has_image;

            // Cari iframe yang benar
            let iframe = document.getElementById("frame-panorama") 
                       || document.getElementById("ipanorama-frame");
            ...
            let interval = setInterval(() => {
                ...
                // cari tombol save builder
                let saveBtn = iframeDoc.querySelector(".ipnrm-ui-cmd-getcode");
                let savePopup = iframeDoc.querySelector(".ipnrm-ui-popup");

                if (!saveBtn) return; // masih loading builder

                clearInterval(interval);

                // ===================================================
                // CASE 1: Tidak ada gambar → blok save
                // ===================================================
                if (!hasImage) {
                    ...
                    saveBtn.addEventListener('click', e => {
                        ...
                        // Tutup popup save yang muncul otomatis
                        if (savePopup) {
                            savePopup.style.display = "none";
                        }

                        alert("Please upload at least 1 panorama image before saving.");
                    });

                    // Antisipasi popup save yang muncul otomatis → tutup setiap 200ms
                    let popupInterval = setInterval(() => {
                        let popup = iframeDoc.querySelector(".ipnrm-ui-popup");
                ...
                // ===================================================
                // CASE 2: Sudah ada gambar → enable save
                // ===================================================
...
// Jalankan setelah iframe load
document.addEventListener("DOMContentLoaded", function () {
    setTimeout(checkPanoramaImagesAndProtectSave, 2000);
});

function fixMissingPanoramaTexture() {
    let iframe = document.getElementById("frame-panorama") || document.getElementById("ipanorama-frame");
    if (!iframe) return;
    ...
    let interval = setInterval(() => {
        ...
        // pastikan sudah siap
        if (!iframeWin || !iframeWin.console) return;

        clearInterval(interval);

        // Backup console.error asli
        const originalError = iframeWin.console.error;

        // Override console.error di dalam iframe
        iframeWin.console.error = function(...args) {

            // Deteksi error texture kosong
            if (args[0] && typeof args[0] === "string" && args[0].includes("Cannot load texture ''")) {
                console.log("Auto-fix: empty texture detected");

                const fallbackImage = "/uploads/media/360-Video-Featured-StudioBinder-Compressed.jpg";

                // Mencari semua scene image
                let imgs = iframe.contentDocument.querySelectorAll("img");

                imgs.forEach(img => {
                    if (!img.src || img.src.trim() === "" || img.src.endsWith("undefined")) {
                        img.src = fallbackImage;
                    }
                });

                // Cegah error keluar ke console
                return;
            }

            // Jika bukan error texture, tetap keluarkan normal
            originalError.apply(this, args);
        };

        console.log("iPanorama texture auto-fix is active.");
    }, 500);
}
...
// Jalankan otomatis
document.addEventListener("DOMContentLoaded", function () {
    setTimeout(fixMissingPanoramaTexture, 2000);
});
