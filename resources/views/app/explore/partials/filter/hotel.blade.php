        ...
        <div class="bravo_search_tour">
            <div class="bravo_filter">
                <form class="bravo_form_filter" action="{{ route('explore.index') }}">
                    ...
                    <!-- Attributes -->
                    @php
                        $hotel_attributes = \Modules\Core\Models\Attributes::where('service', 'hotel')
                            ->orderBy('position', 'desc')
                            ->with(['terms', 'translation'])
                            ->get();
                        $hotel_attribues_selected = (array) Request::query('terms');
                    @endphp
                                ...
                                <div class="item-content">
                                    <ul>
                                        @foreach ($item->terms as $key => $term)
                                            ...
                                            <li @if ($key > 2 and empty($hotel_attribues_selected)) class="hide" @endif>
                                                <div class="bravo-checkbox">
                                                    <label>
                                                        <input @if (in_array($term->id, $hotel_attribues_selected)) checked @endif
                                    ...
                                    @if (count($item->terms) > 3 and empty($hotel_attribues_selected))
                    ...
                    <div class="g-filter-item">
                        <div class="item-content">
                            <div class="form-group mt-3">
                                @php
                                    $hotel_ex = \Modules\Hotel\Models\Hotel::query()
                                        ->select('chain')
                                        ->where([['chain', '!=', ''], ['status', 'publish']])
                                        ->groupBy('chain')
                                        ->get();
                                @endphp
                            ...
                            <div class="from-group mt-3">
                                @php
                                    $hotel_rooms = \Modules\Hotel\Models\Hotel::query()
                                        ->select('room')
                            ...
                            <div class="form-group mt-3">
                                <div class="form-content">
                                    <div class="smart-search d-flex justify-content-between align-items-center">
                                        <input type="text" class="form-control filter_map_place" id="hotel_map_place"
                                            name="map_place" placeholder="Place" data-id="hotel"
                                ...
                                <div class="form-group mt-3">
                                    <input type="hidden" id="hotel_map_lat" name="map_lat"
                                        class="form-control filter_map_lat">
                                    <input type="hidden" id="hotel_map_lgn" name="map_lgn"
                            ...
                            <div class="form-group mt-3">
                                <div class="form-content">
                                    <label class="mb-2">{{ __('Proximity') }} <span
                                            id="hotel_proximity_text">0</span> km</label>
                            ...
                            <div class="form-group mt-3">
                                <label>{{ __('Keyword searchen') }}</label>
                            ...
                            <div class="form-group mt-3">
                                ...
                                <a class="btn btn-secondary form-control p-0"
                                    href="{{ route('explore.index', ['service_type' => 'hotel']) }}"
